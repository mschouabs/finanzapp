...
export default function ConfiguracionPage() {
  ...
  const exportarDatos = () => {
    const datos = { exportadoEn: new Date().toISOString(), preferencias: { moneda, notifGastos, notifMetas, notifResumen } }
  ...
  return (
    <div className="max-w-xl mx-auto space-y-4">
      ...
      <div className="fa-card">
        <button onClick={() => setSeccion(seccion === 'perfil' ? null : 'perfil')} className="w-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-confirm/10 flex items-center justify-center"><User size={16} className="text-confirm" /></div>
        ...
        {seccion === 'perfil' && (
          <div className="mt-4 pt-4 border-t space-y-3">
            <div>
              ...
              <input value={userName} onChange={e => setUserName(e.target.value)} className="w-full px-3 py-2 text-sm rounded-lg border bg-field text-primary focus:outline-none focus:border-confirm" />
            </div>
            <div>
              <label className="text-xs font-semibold text-secondary block mb-1">Email</label>
              <input value={userEmail} disabled className="w-full px-3 py-2 text-sm rounded-lg border bg-field text-secondary opacity-60 cursor-not-allowed" />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="fa-card">
        <button onClick={() => setSeccion(seccion === 'apariencia' ? null : 'apariencia')} className="w-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-confirm/10 flex items-center justify-center"><Palette size={16} className="text-confirm" /></div>
            <div className="text-left">
      ...
      <div className="fa-card">
        <button onClick={() => setSeccion(seccion === 'moneda' ? null : 'moneda')} className="w-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-confirm/10 flex items-center justify-center"><DollarSign size={16} className="text-confirm" /></div>
            <div className="text-left">
              <p className="text-sm font-semibold text-primary">Moneda</p>
              <p className="text-xs text-secondary">Predeterminada: {moneda}</p>
            </div>
          </div>
        ...
        {seccion === 'moneda' && (
          <div className="mt-4 pt-4 border-t">
            ...
            <div className="grid grid-cols-3 gap-2">
              {MONEDAS.map(m => (
                <button key={m} onClick={() => setMoneda(m)} className={`py-2 px-3 rounded-lg text-sm font-semibold border transition-colors ${moneda === m ? 'border-confirm text-confirm bg-confirm/10' : 'border-border text-secondary hover:bg-alternate'}`}>{m}</button>
              ))}
            </div>
          </div>
        )}
      ...
      <div className="fa-card">
        <button onClick={() => setSeccion(seccion === 'notif' ? null : 'notif')} className="w-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-confirm/10 flex items-center justify-center"><Bell size={16} className="text-confirm" /></div>
            <div className="text-left">
          ...
          <div className="mt-4 pt-4 border-t space-y-4">
            {[
              ...
              <div key={label} className="flex items-center justify-between">
                ...
                <button onClick={() => set(!val)} className={`relative w-11 h-6 rounded-full transition-colors ${val ? 'bg-confirm' : 'bg-border'}`}>
      ...
      <div className="fa-card">
        <button onClick={() => setSeccion(seccion === 'privacidad' ? null : 'privacidad')} className="w-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-confirm/10 flex items-center justify-center"><Shield size={16} className="text-confirm" /></div>
            <div className="text-left">
        ...
        {seccion === 'privacidad' && (
          <div className="mt-4 pt-4 border-t space-y-3 text-sm text-secondary leading-relaxed">
            ...
            <p>🗑️ Podés solicitar la eliminación de tu cuenta en cualquier momento.</p>
            <button onClick={exportarDatos} className="mt-2 flex items-center gap-2 text-xs font-semibold text-confirm hover:underline">
              <Download size={14} />
              Exportar mis datos
            </button>
          </div>
      ...
      <button onClick={guardarPreferencias} className="w-full py-3 rounded-xl bg-confirm text-white font-semibold text-sm hover:bg-confirm-hover transition-colors flex items-center justify-center gap-2">
        {guardado ? <><Check size={16} /> ¡Guardado!</> : 'Guardar cambios'}
      </button>

      <button onClick={cerrarSesion} className="w-full py-3 rounded-xl border text-negative font-semibold text-sm hover:bg-negative/10 transition-colors flex items-center justify-center gap-2">
        <LogOut size={16} />
        Cerrar sesión
      </button>
    </div>
